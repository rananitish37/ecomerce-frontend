import React from "react";

function CategoryGrid() {
  return (
    <div className="grid gap-4 grid-rows-12 grid-cols-12 lg:h-[600px] px-5 lg:px-20">
      <div className="col-span-3 row-span-12 text-white">
        <img className="w-full h-full object-cover object-top lg:rounded-md"
          src="https://assets.myntassets.com/h_720,q_90,w_540/v1/assets/images/23807268/2023/6/29/9930b235-5318-4755-abbe-08f99e969e781688026636544LehengaCholi7.jpg"       alt=""
        />
      </div>
      <div className="col-span-2 row-span-6 text-gray-800">
        <img className="w-full h-full object-cover object-top lg:rounded-md"
          src="https://assets.myntassets.com/h_720,q_90,w_540/v1/assets/images/24651572/2023/8/25/4fbf6d8c-d093-46c5-a5a6-7dd67c0c76551692964752597HouseofPataudiMenTanFauxLeatherFormalSlipOnLoafers1.jpg" alt=""
        />
      </div>
      <div className="col-span-4 row-span-6 text-gray-800">
        <img className="w-full h-full object-cover object-top lg:rounded-md"
          src="https://images.pexels.com/photos/12730873/pexels-photo-12730873.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1" alt=""
        />
      </div>
      <div className="col-span-3 row-span-12 text-gray-800">
        <img className="w-full h-full object-cover object-top lg:rounded-md"
          src="https://shreeman.in/cdn/shop/files/20_3cfbd5a3-ecb6-482a-b798-7ffd9de1c784.jpg?v=1712061674&width=700" alt=""
        />
      </div>
      <div className="col-span-4 row-span-6 text-gray-800">
        <img className="w-full h-full object-cover object-top lg:rounded-md"
         src="https://media.istockphoto.com/id/1276740597/photo/indian-traditional-gold-necklace.jpg?b=1&s=612x612&w=0&k=20&c=S-QnNZKqf2u3L-GIaDiIinNRU74GBWQaIDwY7gYJboY="     alt=""
        />
      </div>
      <div className="col-span-2 row-span-6 text-gray-800">
      <img className="w-full h-full object-cover object-top lg:rounded-md"
          src="https://assets.myntassets.com/h_720,q_90,w_540/v1/assets/images/13837166/2021/8/19/04e40e02-4c56-4705-94d0-f444b29973aa1629373611707-House-of-Pataudi-Women-Maroon-Embellished-Handcrafted-Wedges-1.jpg"
          alt=""
        />
      </div>
    </div>
  );
}

export default CategoryGrid;
